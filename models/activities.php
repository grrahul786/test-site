...
//                $result = $db->loadResult();
          ...
          /**********************************************cutom code for activity sort  */
                     ...
                     $activities = $this->_getActivitiesLikeComment($result);
                        ...
                        for($n=0;$n<count($activities);$n++)
                        {
                            $newrow = $activities[$n];
                        
                         $cc = $newrow->getCommentCount();
                         $ll = $newrow->getLikeCount();
        		 $pp = $newrow->getLikeCounts(); 
                         
        
        
                         $sql_count = "select comment_id from #__community_activities where id='".$activities[$n]->id."'";
                         $db->setQuery($sql_count);
                         $comment_id = $db->loadResult();
        
                         $sql_count = "select id from #__community_wall where contentid='".$comment_id."'";
                         $db->setQuery($sql_count);
                         $comment_ids = $db->loadObjectlist();
                         $countprays = 0;   
                         $countlikes = 0; 
                        for($m=0;$m<count($comment_ids);$m++)
                        {
                                         $sql_1 = "select b.like as pray from #__community_likess as b where uid='".$comment_ids[$m]->id."' and element='comment'";
                                         $db->setQuery($sql_1);
                                         $countpray = $db->loadResult();
                        
                                         if($countpray!='')
                                         {
                                         $countpray = explode(',',$countpray);
                                          
                            
                                             $prayval =  count($countpray);
                        ...
                        for($m=0;$m<count($comment_ids);$m++)
                        {
                                         $sql_2 = "select b.like as likes from #__community_likes as b where uid='".$comment_ids[$m]->id."' and element='comment' ";
                                         $db->setQuery($sql_2);
                                         $countlike = $db->loadResult();
                        
                                         if($countlike!='')
                                         {
                                         $countlike = explode(',',$countlike);
                                             $likeval =  count($countlike);
                        ...
                        if(empty($countlike)){
                         ...
                         $countlikes +=  $likeval;
                          ...
                          $totalc = $cc+$pp+$ll+$countprays+$countlikes;
                        
                         $newsql_like = "select act_id,total_counts from #__community_activity_sort where  act_id='".$activities[$n]->id."'";
                         $db->setQuery($newsql_like);
                         $chekactid = $db->loadObjectlist();
                        if($chekactid[0]->act_id == '')
                         {
                           $newsql_like = "insert into #__community_activity_sort (`act_id`, `total_counts`) VALUES ('".$activities[$n]->id."','".$totalc."')";
                           $db->setQuery($newsql_like);
                           $db->execute();
                         }
                         elseif($chekactid[0]->total_counts != $totalc)
                         {
                           $newsql_like = "update #__community_activity_sort set `total_counts`='".$totalc."' where act_id='".$chekactid[0]->act_id."'";
                           $db->setQuery($newsql_like);
                           $db->execute();  
                         }
                         else{
                        ...
                        $sas = "select * from #__community_activity_sort order by total_counts desc";
                            $db->setQuery($sas);
                            $res = $db->loadObjectlist(); 
                            $sort_order = count($res);
                            
                            for($o=0;$o<count($res);$o++)
                            {
                                $db->setQuery("update  #__community_activity_sort set sort_order = '".$sort_order."' where id='".$res[$o]->id."'");
                                $db->execute();
                                $sort_order--;
        ...
        if($_REQUEST['func'] != 'activities,ajaxGetRecentActivitiesCount' && isset($filters['beforeDate'])){
                
                               $last_index = array_pop(array_keys($activities));
                               $last_actid = $activities[$last_index]->id;
                               $db->setQuery("select sort_order from #__community_activity_sort where act_id = '".$last_actid."'");
                               $getlastcountsortorder = $db->loadResult();
                               $_SESSION['last_count'] = $getlastcountsortorder;
                    }
                                    return $activities;
            }
...
// Have limit?
        ...
        //my custom code for sorting  
        
                        if($my->id != 0)
                        {
                            if($my->_groups != '')
                            {
                               $newcondForShare = ' AND (a.' . $db->quoteName('share_gr') . ' IN('.$my->_groups.') OR a.' . $db->quoteName('share_gr') . ' = "" ) ';
                            }
                            else
                            {
                                 $newcondForShare = 'AND a.' . $db->quoteName('share_gr') . ' = ""  ';
                            }
                        }
                        else
                        {
                    ...
                    $db = JFactory::getDbo();
                    $db->setQuery('SELECT enabled FROM #__extensions WHERE name = "activityshare"');
                    $is_Pluginenabled = $db->loadResult();
                    
                    if($is_Pluginenabled) {
                        $addthis_to_sql = ' AND (a.' . $db->quoteName('share_fr') . ' = "" OR a.' . $db->quoteName('actor') . ' = "'.$my->id.'"  OR a.' . $db->quoteName('share_fr') . ' = "'.$my->id.'") '.$newcondForShare ;   //this is for shaing restrictions
                   ...
                   $db=JFactory::getDBO();
$db->setQuery("select `sort_opt` from `#__community_activity_sort_option` where `user_id`='".$useridto."'");
$user_actorder = $db->loadResult();
if($user_actorder!='' && !isset($_REQUEST['act_order']))
{
  $_SESSION['act_order'] = $user_actorder;
}  
        if(isset($_REQUEST['act_order']) || isset($_SESSION['act_order']))
                            {
        
                                $_SESSION['act_order'] = (isset($_REQUEST['act_order']) ? $_REQUEST['act_order'] : $_SESSION['act_order']);
                                $db=JFactory::getDBO();
                                if(isset($_REQUEST['act_order']))
                                {
                                  $useridto = JFactory::getUser()->id;
                                  if($useridto!=0)
                                  {
                                   $db->setQuery("select user_id from `#__community_activity_sort_option` where user_id='".$useridto."'");
                                   $userexist = $db->loadResult();
                                
                                   if($userexist == '')
                                   {
                                    $db->setQuery("INSERT INTO `#__community_activity_sort_option`(`user_id`, `sort_opt`) VALUES ('".$useridto."','".$_REQUEST['act_order']."')");
                                    $db->execute();
                                   }
                                   else
                                   {
                                    $db->setQuery("UPDATE `#__community_activity_sort_option` SET `sort_opt`='".$_REQUEST['act_order']."' WHERE user_id='".$useridto."'");
                                    $db->execute();
                                   }
                                
                                  }
                                }
                                if($_SESSION['act_order'] =='desc')
                                {
                                $orderchanged = 'b.total_counts  '.$_SESSION['act_order'].', ';
                                }
                                else{
                                   $orderchanged = '';
                                }
                                
                                if (isset($filters['beforeDate']) && !empty($filters['beforeDate']) && $_SESSION['act_order'] =='desc'){
                                    if($_SESSION['last_count'] == '')
                                    {
                                               $db->setQuery("select sort_order from #__community_activity_sort order by sort_order desc");
                                               $getlastcountsortorder = $db->loadResult();
                                     $_SESSION['last_count'] = $getlastcountsortorder-20;
                                    }
                                    $whereAndadd = ' AND b.sort_order < '.$_SESSION['last_count'] ;
                                                                       $replaceable = 'AND a.created <= '.$db->Quote($filters['beforeDate']);
                                                                       $whereAnd = str_replace($replaceable,'',$whereAnd); 
                                                                       $whereAnd = str_replace('AND '.$exclusionQuery,'',$whereAnd); 
                                                                       $whereAnd .= $whereAndadd;
                                }         
        
        
                                $sql = 'SELECT a.*,b.total_counts,b.id as sortid'
                            /* .' TO_DAYS('.$db->Quote($date->toSql(true)).') -  TO_DAYS( DATE_ADD(a.' . $db->quoteName('created').', INTERVAL '.$date->getOffset(true).' HOUR ) ) as _daydiff' */
                            . ' FROM ' . $db->quoteName('#__community_activities') . ' as a left join #__community_activity_sort as b on b.act_id=a.id'
                            . ' WHERE '
                            /* AND with groupped OR */
                            . $whereOr
                            /* AND with groupped AND */
                             ...
                             * @todo Need re-apply for 3.3 branch
                             */
                            . ' AND a.' . $db->quoteName('archived') . ' = 0  '.$addthis_to_sql.' '
                            . ' GROUP BY a.' . $db->quoteName('id')
                            . ' ORDER BY '.$orderchanged.' a.'. $db->quoteName('id').' DESC, a.'. $db->quoteName('created'). ' DESC' .  $maxEntries;
                            ...
                            . ' AND a.' . $db->quoteName('archived') . ' = 0  '.$addthis_to_sql.' '
                            . ' GROUP BY a.' . $db->quoteName('id')
                            . ' ORDER BY a.'. $db->quoteName('id').' DESC, a.'. $db->quoteName('created'). ' DESC' . $maxEntries;
                ...
                if (!empty($result)) {
                    ...
                    if (!empty($cond)) {
                      ...
                      if (!empty($cond)) {
                        ...
                        $sql = 'SELECT a.* '
                            . ' FROM ' . $db->quoteName('#__community_likess') . ' as a '
                            . ' WHERE '
                            . implode(' OR ', $cond);

                        $db->setQuery($sql);
                        $resultLikess = $db->loadObjectList();
						
                        if ($db->getErrorNum()) {
                            JError::raiseError(500, $db->stderr());
                        }
                        foreach ($resultLikess as $likes) {

                            $likesResults[$likes->element . '-' . $likes->uid] = $likes->like;
                        }
                    }
                ...
                if (!empty($result)) {
                    foreach ($result as $row) {
                        ...
                        if (array_key_exists($row->like_type . '-' . $row->like_id, $likesResult)) {
                        	...
                        	if (array_key_exists($row->like_type . '-' . $row->like_id, $likesResults)) {
                            $row->_pray = $likesResults[$row->like_type . '-' . $row->like_id];
                        } else {
                            $row->_pray = '';
                        }
